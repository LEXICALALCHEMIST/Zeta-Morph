...
export default class Unit5 {
  constructor() {
    this.state = {
      label: 'unit5', // Ten-thousands unit
      currentSymbol: null,
      pushes: [],
      carry: 0,
      collapsed: false,
    ...
    this.carryTarget = 'Unit4'; // Carry to Unit4
  }

  push(times = 1, carryBus) {
    if (times >= 10) {
      console.log(`Unit5 Push: Bypassing ${times} to collapse`);
      ...
      if (carryBus) {
        ...
        console.log(`Unit5 Carry: Registered CARRY: ${this.state.carry} to ${this.carryTarget}`);
      }
      this.state.pushes = [];
      return;
    }
    ...
    for (let i = 0; i < absTimes; i++) {
      ...
      console.log(`Unit5 Push: SYMBOL: ${this.state.currentSymbol} CARRY: ${this.state.carry} DIRECTION: ${direction}`);
      if (this.state.currentSymbol === SYMBOL_SEQUENCE[10] && direction > 0) {
        this.state.collapsed = true;
        this.state.hasCollapsed = true;
        this.state.carry = 1;
        console.log(`Unit5 Carry: CARRY: ${this.state.carry} COLLAPSED: ${this.state.collapsed}`);
        if (carryBus) {
          carryBus.registerCarry(1, this.carryTarget);
          console.log(`Unit5 Carry: Registered CARRY: 1 to ${this.carryTarget}`);
        }
        this.state.currentSymbol = SYMBOL_SEQUENCE[0];
        this.state.pushes = [];
      }
